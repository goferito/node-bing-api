  ...
  this.searchVertical = function(query, vertical, callback, options) {
    ...
    var reqUri = opts.rootUri
                   ...
                   + (opts.imagefilters 
                       ...
                       : '')
                   + (opts.newsCategory
                       ? '&' + qs.stringify({ "NewsCategory": "'" + opts.newsfilters.category + "'" })
                       : '')
                   + (opts.newsLocationOverride
                       ? '&' + qs.stringify({ "NewsLocationOverride": "'" + opts.newsfilters.locationOverride + "'" })
                       : '')
                   + (opts.newsSortBy
                       ? '&' + qs.stringify({ "NewsSortBy": "'" + opts.newsfilters.sortBy + "'" })
                       : '');
...
Bing.prototype.news = function(query, callback, options) {
    if (options && options.newsfilters) {
      if (options.newsfilters.category) {
        options.newsfilters.category = 'rt_' 
          + capitalizeFirstLetter(options.newsfilters.category);
      }
      if (options.newsfilters.sortBy) {
        options.newsfilters.sortBy = 
          capitalizeFirstLetter(options.newsfilters.sortBy);
      }
      if (options.newsfilters.newsLocationOverride) {
        options.newsfilters.newsLocationOverride = 
          capitalizeFirstLetter(options.newsfilters.newsLocationOverride);
      }
    }
